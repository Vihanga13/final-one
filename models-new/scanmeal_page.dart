...
class _ModernScanMealPageState extends State<ScanMealPage> with TickerProviderStateMixin {
  ...
  Future<void> _pickImageFromGallery() async {
    ...
    if (image != null) {
      ...
      await _processImageAndSave(_selectedImage!);
    }
  }

  Future<void> _processImageAndSave(File image) async {
    setState(() {
      _isScanning = true;
    });
    ...
    try {
      ...
      var request = http.MultipartRequest(
        'POST',
        Uri.parse('https://kaluu13-finalproject.hf.space/predict'), // Use your server IP
      );
  ...
  void _scanMeal() async {
    try {
      if (_controller != null && _controller!.value.isInitialized) {
        ...
        await _processImageAndSave(imageFile);
      }
    } catch (e) {
